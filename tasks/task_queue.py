    ...
    try:
        ...
        if last_sent is not None:
            ...
            if elapsed < RATE_LIMIT_SECONDS:
                log_warning(f"Rate limit enforced: waiting {RATE_LIMIT_SECONDS - elapsed} seconds before sending email.")
...
def send_daily_report(email: str, profession, name: str):
    ...
    log_info(f"Job triggered for {email}, {profession}, {name}")
    wait_for_rate_limit()
    log_info(f"Continuing to send report for {email}, {profession}, {name}")
